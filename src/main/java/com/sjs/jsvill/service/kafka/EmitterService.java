...
public class EmitterService {
    private final ProdNotiService notificationsService;
    private final EmitterRepository emitterRepository;

    private static final int MAX_NOTIFICATIONS_COUNT = 6;

    //SSE 연결된 클라이언트에게 실시간으로 알림을 전달한다.
    @KafkaListener(topics = "calendar-noti", groupId = "group_1")
    public void listen(NotiMessage notiMessage) {
        //인위적으로 폴링 간격을 늘림
        ...
        Json.stringToJson(notiMessage, "KafkaListener-listen");
